...
### Token Types
 ...
 - **Register Address**:
    ...
    - Token Explanation: represents a single byte register that can be used as a parameter to ALU operations. The `M` register (memory address) is not actually a physical register and will use the memory address specified by the `H` and `L` registers for any operation expecting a register parameter. You can store to this location or pull from it, but some instructions prohibit doing both at the same time.
 - **Byte Literal**:
    - *Regex*: `-?[0-9]+`
    - Regular Expression Explanation: matches a decimal number without separators that can be either positive or negative.
    - Token Explanation: a decimal number representing a single byte of immediate data. This is converted to an unsigned character and takes only one byte to store.
 - **RAM Address**: 
    - *Regex*: `[rR][0-9]+`
...
- **Pseudo-Instructions**:
    ...
    - Token Explanation: an operation that is not translatable to machine code and is strictly an assembler function. All functions have aliases in both camelCase and SCREAMING_SNAKE_CASE. The only currently defined pseudo-instructions are: 
        ...
        takes in an instruction, memory address or variable representing a memory address, and byte offset. For example `.relJump JMP 15 -3` Adds the instruction to the compiled machine code like normal and computes the sum of the memory address and offset before pushing the address to the compiled machine code. This can be used to jump to a named position with an offset to cut down on named position count.
